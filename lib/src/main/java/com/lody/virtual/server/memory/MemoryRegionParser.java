...
public class MemoryRegionParser {
     ...
     * <p>
     * ([0-9a-f]+)  -   ([0-9a-f]+)  \s  ([r-]) ([w-]) ([x-]) ([sp])   \s    ([0-9a-f]+)   \s
     * ([0-9a-f]+) :   ([0-9a-f]+)     \s  (\d+)   \s?  (.*) StartAddress        EndAddress    Read
     *  Write  Execute Shared        Filemap Offset   Major devnum    Minor devnum         Inode
     * Description
     */
